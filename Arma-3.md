...
```
The issue that makes this difficult is that, for some bizarre reason, demands that the modules possess all lowercase names.  The following code (run from the `serverfiles/mods` directory *as the arma3server user*) changes all files and folders to lowercase:
```
depth=0
